// News Digest Constants - Options for News Digest workflow customization
...
// Helper to get days from time period ID
export function getTimePeriodDays(id: string): number {
  const period = TIME_PERIODS.find(p => p.id === id);
  return period?.days || 30;
}

// Helper to get output style instructions
export function getOutputStyleInstructions(id: string): string {
  const styleMap: Record<string, string> = {
    'executive-brief': 'Use bullet points, lead with key metrics, keep paragraphs short and scannable.',
  ...
  return styleMap[id] || styleMap['executive-brief'];
}
