...
export function getProductsDescription(productIds: string[]): string {
  return CISCO_IIOT_PRODUCTS.filter((p) => productIds.includes(p.id))
    .map((p) => `${p.name}: ${p.keyProducts.join(', ')}`)
    .join('\n');
}

export function getIndustryDescription(industryId: string): string | null {
  const industry = getIndustryById(industryId);
  if (!industry) return null;
  return `${industry.name}: ${industry.description}\nKey use cases: ${industry.useCases.join(', ')}`;
}
