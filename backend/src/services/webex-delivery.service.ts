...
import { AttachmentActionDetails } from '../types/webex-webhook.types';
...
_Generated by IIoT Account Intelligence_`;
  ...
  /**
   * Send an Adaptive Card via Webex
   */
  async sendCardMessage(
    ...
    logger.info('Webex card sent successfully', { destination, destinationType });

    return response.json() as Promise<WebexMessageResponse>;
  }

  /**
   * Get attachment action details (form submission data)
   */
  async getAttachmentAction(actionId: string): Promise<AttachmentActionDetails> {
    // Get Webex token from admin settings
    const adminService = getAdminService();
    const settings = await adminService.getSettings();
    ...
    const response = await fetch(`${WEBEX_API_BASE_URL}/attachment/actions/${actionId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${settings.webexBotToken}`,
      },
    ...
    logger.info('Attachment action fetched successfully', { actionId });

    return response.json() as Promise<AttachmentActionDetails>;
  }
}
